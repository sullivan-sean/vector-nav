...
class GridScorer(object):
    ...
    """
        ...
        # Create all masks
        self._masks = [(self._get_ring_mask(mask_min,
                                            mask_max), (mask_min, mask_max))
    ...
    def calculate_ratemap(self, xs, ys, activations, statistic='mean'):
        return scipy.stats.binned_statistic_2d(xs,
                                               ys,
                                               activations,
                                               bins=self._nbins,
                                               statistic=statistic,
    ...
    def grid_score_60(self, corr):
        if self._min_max:
            ...
            return (corr[60] + corr[120]) / 2 - (corr[30] + corr[90] +
                                                 corr[150]) / 3
    ...
    def get_grid_scores_for_mask(self, sac, rotated_sacs, mask):
        ...
        for angle, rotated_sac in zip(self._corr_angles, rotated_sacs):
            ...
            cross_prod = np.sum(
                masked_sac_centered * masked_rotated_sac) / ring_area
